...
public interface CarteFideliteRepository extends JpaRepository<CarteFidelite, Long> {
    ...
    // 14. Trouver les cartes dont le client fête son anniversaire aujourd'hui
    @Query("SELECT c FROM CarteFidelite c " +
            "WHERE DAY(c.client.dateNaissance) = DAY(CURRENT_DATE) " +
            "AND MONTH(c.client.dateNaissance) = MONTH(CURRENT_DATE)")
    List<CarteFidelite> findCartesAnniversaireAujourdhui();
